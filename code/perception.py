...
def find_rocks(img):
    # find rock by using RGB value
    rockpixels = ((img[:, :, 0] > 100)
                  & (img[:, :, 1] > 100)
                  & (img[:, :, 2] < 50))
    # Convert TRUE -> 1
    return rockpixels.astype(int)


# Apply the above functions in succession and update the Rover state accordingly
def perception_step(Rover):
    # USE examples what I've learned in previous lecture!
    dst_size = 5
    bottom_offset = 6
    image = Rover.img
    ...
    # 장애물을 처리한다.
    obs_map = np.around(np.absolute(np.float32(threshed)-1))
    ...
    if rock_map.any():
        ...
        rock_idx = int((np.argmin(rock_dist) + np.argmax(rock_dist)) / 2)
